...
func TestHealthz(t *testing.T) {
	rec := httptest.NewRecorder()
	newAdminServer(&readiness{}, slog.New(slog.DiscardHandler), allOK).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/healthz", nil))
...
func TestReadyz(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rec := httptest.NewRecorder()
			newAdminServer(r, slog.New(slog.DiscardHandler), tt.checks).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/readyz", nil))
