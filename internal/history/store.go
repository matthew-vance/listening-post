...
// (event_station_id, event_id, event_ts, last_seen) unique constraint so an at-least-once re-read is idempotent.
const insertSQL = `
INSERT INTO aircraft_traces (
    event_station_id, event_id, event_ts, icao, callsign, altitude, ground_speed, track, lat, lon, vertical_rate, squawk,
...
func (w *writer) insert(rows []traceRow) error {
	...
	for _, r := range rows {
		batch.Queue(insertSQL,
			r.EventStationID, r.EventID, r.EventTS, r.Icao, r.Callsign, r.Altitude, r.GroundSpeed, r.Track, r.Lat, r.Lon, r.VerticalRate, r.Squawk,
