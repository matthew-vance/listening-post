...
The gateway (`gateway/`, Go) runs on the server via `docker compose` (`just up`) behind Traefik, and currently just validates and logs incoming batches. Its health probes are on a separate admin port that only Traefik can reach.

Traefik is there to terminate TLS. The Let's Encrypt configuration is present in `compose.yaml` but commented out until there is a real hostname.
